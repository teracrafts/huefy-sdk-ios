...
# Deploy Laravel SDK to standalone repository using git subtree
# Usage: ./scripts/deploy-laravel-subtree.sh [version]

set -e

# Configuration
REMOTE_REPO="git@github.com:teracrafts/huefy-sdk-laravel.git"
SUBDIRECTORY="sdks/laravel"
VERSION=${1:-"v1.0.0"}
...
echo -e "${BLUE}🚀 Deploying Laravel SDK to standalone repository${NC}"
...
if [[ ! -d "sdks/laravel" ]]; then
    ...
    echo -e "${RED}   Current directory should contain 'sdks/laravel'${NC}"
    exit 1
fi
...
if ! git remote get-url laravel-repo >/dev/null 2>&1; then
    echo -e "${YELLOW}🔗 Adding remote repository...${NC}"
    git remote add laravel-repo "$REMOTE_REPO"
else
    echo -e "${YELLOW}🔗 Updating remote repository URL...${NC}"
    git remote set-url laravel-repo "$REMOTE_REPO"
fi
...
if git push laravel-repo "${SUBTREE_COMMIT}:refs/heads/main" --force; then
...
if git push laravel-repo "${SUBTREE_COMMIT}:refs/tags/$VERSION" --force; then
...
if git ls-remote laravel-repo | grep -q "refs/tags/$VERSION"; then
...
echo ""
echo -e "${GREEN}🎉 Laravel SDK deployment completed successfully!${NC}"
echo ""
echo -e "${BLUE}📦 Package Information:${NC}"
echo -e "${BLUE}   Package: teracrafts/huefy-laravel${NC}"
echo -e "${BLUE}   Version: $VERSION${NC}"
echo -e "${BLUE}   Repository: $REMOTE_REPO${NC}"
echo ""
echo -e "${BLUE}📋 Usage Instructions:${NC}"
echo -e "${BLUE}   composer require teracrafts/huefy-laravel${NC}"
echo ""
echo -e "${BLUE}🔗 Repository URL:${NC}"
echo -e "${BLUE}   https://github.com/teracrafts/huefy-sdk-laravel${NC}"

# Cleanup
git remote remove laravel-repo >/dev/null 2>&1 || true
