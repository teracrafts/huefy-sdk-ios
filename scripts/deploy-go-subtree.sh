...
# Deploy Go SDK to standalone repository using git subtree
# Usage: ./scripts/deploy-go-subtree.sh [version]
...
echo -e "${BLUE}🚀 Deploying Go SDK to standalone repository${NC}"
...
if [[ ! -d "sdks/go" ]]; then
    ...
    echo -e "${RED}   Current directory should contain 'sdks/go'${NC}"
    exit 1
fi
...
if ! git remote get-url go-repo >/dev/null 2>&1; then
    echo -e "${YELLOW}🔗 Adding remote repository...${NC}"
    git remote add go-repo "$REMOTE_REPO"
else
    echo -e "${YELLOW}🔗 Updating remote repository URL...${NC}"
    git remote set-url go-repo "$REMOTE_REPO"
fi
...
if git push go-repo "${SUBTREE_COMMIT}:refs/heads/main" --force; then
...
if git push go-repo "${SUBTREE_COMMIT}:refs/tags/$VERSION" --force; then
...
if git ls-remote go-repo | grep -q "refs/tags/$VERSION"; then
...
echo -e "${BLUE}   Module: github.com/teracrafts/huefy-sdk-go/v2${NC}"
echo -e "${BLUE}   Version: $VERSION${NC}"
echo -e "${BLUE}   Repository: $REMOTE_REPO${NC}"
echo ""
echo -e "${BLUE}📋 Usage Instructions:${NC}"
echo -e "${BLUE}   go get github.com/teracrafts/huefy-sdk-go/v2@$VERSION${NC}"
echo ""
