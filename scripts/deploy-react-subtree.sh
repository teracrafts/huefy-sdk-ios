...
# Deploy React SDK to standalone repository using git subtree
# Usage: ./scripts/deploy-react-subtree.sh [version]

set -e

# Configuration
REMOTE_REPO="git@github.com:teracrafts/huefy-sdk-react.git"
SUBDIRECTORY="sdks/react"
VERSION=${1:-"v1.0.0"}
...
echo -e "${BLUE}🚀 Deploying React SDK to standalone repository${NC}"
...
if [[ ! -d "sdks/react" ]]; then
    ...
    echo -e "${RED}   Current directory should contain 'sdks/react'${NC}"
    exit 1
fi
...
if ! git remote get-url react-repo >/dev/null 2>&1; then
    echo -e "${YELLOW}🔗 Adding remote repository...${NC}"
    git remote add react-repo "$REMOTE_REPO"
else
    echo -e "${YELLOW}🔗 Updating remote repository URL...${NC}"
    git remote set-url react-repo "$REMOTE_REPO"
fi
...
if git push react-repo "${SUBTREE_COMMIT}:refs/heads/main" --force; then
...
if git push react-repo "${SUBTREE_COMMIT}:refs/tags/$VERSION" --force; then
...
if git ls-remote react-repo | grep -q "refs/tags/$VERSION"; then
...
echo ""
echo -e "${GREEN}🎉 React SDK deployment completed successfully!${NC}"
echo ""
echo -e "${BLUE}📦 Package Information:${NC}"
echo -e "${BLUE}   Package: @teracrafts/huefy-react${NC}"
echo -e "${BLUE}   Version: $VERSION${NC}"
echo -e "${BLUE}   Repository: $REMOTE_REPO${NC}"
echo ""
echo -e "${BLUE}📋 Usage Instructions:${NC}"
echo -e "${BLUE}   npm install @teracrafts/huefy-react@$VERSION${NC}"
echo ""
echo -e "${BLUE}🔗 Repository URL:${NC}"
echo -e "${BLUE}   https://github.com/teracrafts/huefy-sdk-react${NC}"

# Cleanup
git remote remove react-repo >/dev/null 2>&1 || true
