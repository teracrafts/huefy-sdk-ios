...
# Deploy JavaScript SDK to standalone repository using git subtree
# Usage: ./scripts/deploy-javascript-subtree.sh [version]

set -e

# Configuration
REMOTE_REPO="git@github.com:teracrafts/huefy-sdk-js.git"
SUBDIRECTORY="sdks/javascript"
VERSION=${1:-"v1.0.0"}
...
echo -e "${BLUE}🚀 Deploying JavaScript SDK to standalone repository${NC}"
...
if [[ ! -d "sdks/javascript" ]]; then
    ...
    echo -e "${RED}   Current directory should contain 'sdks/javascript'${NC}"
    exit 1
fi
...
if ! git remote get-url javascript-repo >/dev/null 2>&1; then
    echo -e "${YELLOW}🔗 Adding remote repository...${NC}"
    git remote add javascript-repo "$REMOTE_REPO"
else
    echo -e "${YELLOW}🔗 Updating remote repository URL...${NC}"
    git remote set-url javascript-repo "$REMOTE_REPO"
fi
...
if git push javascript-repo "${SUBTREE_COMMIT}:refs/heads/main" --force; then
...
if git push javascript-repo "${SUBTREE_COMMIT}:refs/tags/$VERSION" --force; then
...
if git ls-remote javascript-repo | grep -q "refs/tags/$VERSION"; then
...
echo ""
echo -e "${GREEN}🎉 JavaScript SDK deployment completed successfully!${NC}"
echo ""
...
echo -e "${BLUE}   npm install @teracrafts/huefy@$VERSION${NC}"
echo ""
echo -e "${BLUE}🔗 Repository URL:${NC}"
echo -e "${BLUE}   https://github.com/teracrafts/huefy-sdk-js${NC}"

# Cleanup
git remote remove javascript-repo >/dev/null 2>&1 || true
